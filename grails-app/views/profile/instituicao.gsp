...
<body>
	...
	<div id="breadcrumb">
		<div class="container">
			...
			<div id="breadcrumb-info">
				<g:link controller='pesquisa' action="index">Centro médico</g:link> <span>&raquo;</span> 
	...
	<div class="container content">
		...
		<div class="content-middle profile-info">
			<h1>${usuario?.nome}</h1>
			<h2>Diretor médico: ${responsavel?.nome} ${responsavel?.sobrenome}</h2>
			<h3>CRM-MG ${responsavel?.codigoConselhoRegional}</h3>
			
			<div class="form-actions">
				<g:if test="${usuario?.email == sec.loggedInUserInfo(field:'username').toString()}">
					<a id="profile-edit-btn" href="${createLink(controller: 'cadastro', action: 'cadastroAdicionalInstituicao')}" class="btn" title="Editar perfil">
			...
			<div class="info">${instituicao?.especialidade}</div>

			<h4><i class="icon-certificate"></i> Patologia(s):</h4>
			<div class="info">${instituicao?.patologia}</div>

			<h4><i class="icon-list-alt"></i> Histórico:</h4>
			<div class="info">${instituicao?.historico}</div>
		</div>
